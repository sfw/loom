...
def _supportable_claim_for_source(path: str) -> str:
    suffix = Path(path).suffix.lower()
    if suffix == ".csv":
        return "sample,1"
    if suffix == ".json":
        return '"status": "ok"'
    return "Generated by Loom process contract testing."
...
def _executor_responses(process: ProcessDefinition) -> list[ModelResponse]:
    ...
    for phase in process.phases:
        ...
        synthesis_dep_files: list[str] = []
        if phase.is_synthesis:
            for dep_id in phase.depends_on:
                dep_name = str(dep_id).strip()
                if not dep_name:
                    continue
                synthesis_dep_files.extend(
                    str(path).strip()
                    for path in deliverables.get(dep_name, [])
                    if str(path).strip()
                )
        phase_validity_contract = process.resolve_validity_contract_for_phase(
            phase.id,
            is_synthesis=bool(phase.is_synthesis),
        )
        require_fact_checker = bool(
            isinstance(phase_validity_contract, dict)
            and phase_validity_contract.get("require_fact_checker_for_synthesis", False),
        )
        if files:
            ...
            if phase.is_synthesis and require_fact_checker:
                source_refs = list(dict.fromkeys(
                    [*synthesis_dep_files, *files]
                ))
                fact_checker_args: dict[str, object] = {
                    "claims": [
                        _supportable_claim_for_source(source_refs[0])
                        if source_refs
                        else "Generated by Loom process contract testing."
                    ],
                }
                if source_refs:
                    fact_checker_args["sources"] = source_refs[:8]
                tool_calls.append(ToolCall(
                    id=f"{phase.id}-fact-check",
                    name="fact_checker",
                    arguments=fact_checker_args,
                ))
