...
- Reviewed model selection and invocation paths in:
  ...
  - `<repo-root>/src/loom/engine/semantic_compactor.py`
  - `<repo-root>/src/loom/__main__.py`
  - `<repo-root>/src/loom/tui/app.py`
  - `<repo-root>/src/loom/cowork/session.py`
  - `<repo-root>/src/loom/learning/reflection.py`

## Findings
### Compliant (role-routed)
- Router is role-first (`role -> candidates -> tier preference`):
  - `<repo-root>/src/loom/models/router.py:57`
- Planner/replanner use `role="planner"`:
  - `<repo-root>/src/loom/engine/orchestrator.py:1437`
  - `<repo-root>/src/loom/engine/orchestrator.py:1720`
- Subtask execution uses `role="executor"`:
  - `<repo-root>/src/loom/engine/runner.py:280`
- Memory extraction uses `role="extractor"`:
  - `<repo-root>/src/loom/engine/runner.py:636`
- Verification uses `role="verifier"`:
  - `<repo-root>/src/loom/engine/verification.py:1762`
  - `<repo-root>/src/loom/engine/verification.py:1709`
- Default cowork model selection uses `role="executor"`:
  - `<repo-root>/src/loom/__main__.py:286`
  - `<repo-root>/src/loom/tui/app.py:712`
...
- `--model` path returns provider by name directly with no role check:
  - `<repo-root>/src/loom/__main__.py:279`
  - `<repo-root>/src/loom/__main__.py:280`
  - `<repo-root>/src/loom/__main__.py:281`
...
- Direct calls to `self._model.complete(...)`:
  - `<repo-root>/src/loom/tui/app.py:2073`
  - `<repo-root>/src/loom/tui/app.py:2109`
...
- Direct call to `self._model.complete(...)`:
  - `<repo-root>/src/loom/tui/app.py:3293`
...
- Requested role falls back to `verifier`, then `executor`:
  - `<repo-root>/src/loom/engine/semantic_compactor.py:99`
...
- Direct calls to `_model.complete(...)`:
  - `<repo-root>/src/loom/learning/reflection.py:388`
  - `<repo-root>/src/loom/learning/reflection.py:454`
